#!/usr/bin/env bash
# 功能：一键执行 Xerxes 的 Debug 构建，并可继续触发 gem5 的 debug 构建。
# 默认开启调试符号，并按 CPU 核数和当前可用内存自动估算并行度。
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD_DIR="${1:-${ROOT_DIR}/debug}"
GEM5_TARGET="${GEM5_TARGET:-X86_MESI_Two_Level_Xerxes}"
GEM5_BINARY="${GEM5_BINARY:-gem5.debug}"
...
echo "Debug build finished: ${BUILD_DIR}/Xerxes"
if [ "${BUILD_GEM5}" != "0" ]; then
  echo "gem5 debug build finished: ${ROOT_DIR}/third_party/gem5/build/${GEM5_TARGET}/${GEM5_BINARY}"
fi
